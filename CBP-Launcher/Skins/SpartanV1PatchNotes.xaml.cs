...
namespace CBPLauncher.Skins
{
    ...
    /// WARNING
    public partial class SpartanV1PatchNotes : UserControl
    {
        ...
        private void FDViewer_Initialized(object sender, EventArgs e)
        {
            if (IsInDesignMode() == false)
            {
                ...
                string placeholderHtml = "<html><body style='background-color: #DFF3F3F3; font-family: sans-serif; color: #FF1A1A1A;'>";
                ...
                worker.DoWork += (s, ev) =>
                {
                    try
                    {
                        ...
                        {
                            string formattedPatchNotes = "<html><body style='background-color: #DFF3F3F3; font-family: sans-serif; color: #FF1A1A1A;'>" + ProcessBBCodeFromTxtFile(patchnotes) + "</body></html>";
